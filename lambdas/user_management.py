    ...
    try:
        ...
        if target_user_role and not user_groups.get(target_user_role):
            return {
                ...
                "body": json.dumps({"error": "Invalid target_user_role format"})
            }
    ...
    try:
        ...
        if not target_user_id:
            return {
                    "statusCode": 400,
                    "headers": cors_headers(),
                    "body": json.dumps({"error": "Missing required field: target_username"})
                }
        if not target_user_role:
            return {
                    "statusCode": 400,
                    "headers": cors_headers(),
                    "body": json.dumps({"error": "Missing required field: target_user_role"})
                }
        if not user_groups.get(target_user_role):
            return {
                    "statusCode": 400,
                    "headers": cors_headers(),
                    "body": json.dumps({"error": "Invalid format for: target_user_role"})
                }
        ...
        if user_groups[target_user_role] >= user_groups[caller_role]:
            return {
                    "statusCode": 403,
                    "headers": cors_headers(),
                    "body": json.dumps({"error": "Invalid permissions to set user to specified target_user_role"})
                }
