...
# Table references
log_hour_table = dynamodb.Table(os.environ.get("DEVICE_TRAIL_LOG_HOUR_TABLE", "local_device_trail_hour_table"))
log_day_table = dynamodb.Table(os.environ.get("DEVICE_TRAIL_LOG_DAY_TABLE", "local_device_trail_day_table"))
log_week_table = dynamodb.Table(os.environ.get("DEVICE_TRAIL_LOG_WEEK_TABLE", "local_device_trail_week_table"))
log_month_table = dynamodb.Table(os.environ.get("DEVICE_TRAIL_LOG_MONTH_TABLE", "local_device_trail_month_table"))
trail_table = dynamodb.Table(os.environ.get("TRAIL_TABLE", "local_trail_table"))
device_table = dynamodb.Table(os.environ.get("DEVICE_TABLE", "local_device_table"))
device_trail_table = dynamodb.Table(os.environ.get("DEVICE_TRAIL_TABLE", "local_device_trail_table"))
...
def get_weather_multiplier(latitude, longitude):
    try:
        ...
        url_request = urllib.request.Request(url, headers={"User-Agent": "AWA TrailPlanner"})

        with urllib.request.urlopen(url_request) as response:
            url_data = json.loads(response.read())

        weather_request = urllib.request.Request(url_data["properties"]["forecast"], headers={"User-Agent": "AWA TrailPlanner"})
