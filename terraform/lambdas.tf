...
resource "aws_lambda_function" "simulate_data" {
  function_name = "${var.deploy_env}_traillcount_simulate_data"
...
resource "aws_cloudwatch_event_rule" "trigger_simulate_data" {
    name = "${var.deploy_env}_traillcount_trigger_simulate_data"
...
resource "aws_cloudwatch_event_target" "trigger_lambda_every_day" {
    ...
    target_id = "${var.deploy_env}_traillcount_simulate_data_event"
...
resource "aws_lambda_function" "cleanup_lambda" {
  function_name = "${var.deploy_env}_traillcount_cleanup_csv_bucket"
  role          = aws_iam_role.lambda_iam_role.arn
